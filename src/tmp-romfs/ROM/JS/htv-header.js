...
function tab(msg) {
	msg = msg.replace(' ','&nbsp;')
	if (msg) {
		d.write('<td width=100% height=80 valign=top background=file://ROM/Images/Themes/ShadowLogo.gif novtilebg><td abswidth=460 height=54 valign=top background=file://ROM/Images/Themes/ShadowLogo.gif align=right novtilebg><spacer height=32 type=block><b><shadow><blackface><font color=cbcbcb>'+msg+' &nbsp; </font></blackface></shadow></b>')
	} else {
		d.write('<td width=100% height=80 valign=top align=left background="file://ROM/Images/Themes/ShadowLogo.gif" novtilebg>')
	}
}
...
function butt(th, v, n, w, t, x) {
	...
	if (x) d.write(' '+x)
	if (th == 1) {
		d.write(' usestyle borderimage=file://ROM/Borders/ButtonBorder4.bif')
	} else if (th == 2 || th == 3) {
		d.write(' usestyle borderimage=file://ROM/Borders/ButtonBorderTh'+th+'.bif')
	} 
	d.write('></font>');
}
